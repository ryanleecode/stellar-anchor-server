...
type TransactionFactory interface {
	Build(serverAccount Account, clientAccount Account) (*txnbuild.Transaction, error)
}
...
func NewService(fact TransactionFactory, kp *keypair.Full, passphrase string) *Service {
	return &Service{
		txFactory: fact,
		keypair:            kp,
...
func (s *Service) BuildSignEncodeChallengeTransactionForAccount(id string) (string, error) {
	...
	txn, err := s.txFactory.Build(&serverAccount, &clientAccount)
...
func (s *Service) ValidateClientSignedChallengeTransaction(
	...
	if err != nil {
		return validationErrs, errors.Wrap(err, "cannot hash transaction")
	}
